...
def prepare_behandler() -> pd.DataFrame:
	...
	df["key_name_sorted"] = df["key_name"].map(lambda s: " ".join(sorted(s.split())))
...
def build(
	...
	for _, p in people.iterrows():
		...
		if match is None:
			...
			cands = [
				r for _, r in beh.iterrows() if want and r["key_tokens"] and want <= r["key_tokens"]
			] or [
				r
				for _, r in beh.iterrows()
				if r["key_tokens"]
				and r["key_tokens"] <= frozenset(p["key_name"].split())
		...
		if match is None and p["key_short"]:
			pref = p["key_short"][:10]
			cands = [r for _, r in beh.iterrows() if pref and r["key_name"].replace(" ", "").startswith(pref)]
