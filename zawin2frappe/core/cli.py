...
def _show(df: pd.DataFrame, limit: int | None = 40) -> None:
    with pd.option_context(
        "display.max_rows", limit or 10_000,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    t.add_argument("--candidates", action="store_true",
                   help="restrict to the agenda/HR domain vocabulary")
    ...
    a.add_argument("--archive", action="store_true",
                   help="include TAGPLANTERMINARCHIV (2009-2017)")
    ...
    bl.add_argument("target", choices=["employees", "assignments", "all"], default="all",
                    nargs="?")
    dfrom, dto = _default_window()
    bl.add_argument("--date-from", default=dfrom,
                    help=f"assignment period start (default {dfrom}: trailing 12 months)")
    bl.add_argument("--date-to", default=dto, help=f"assignment period end (default {dto})")
    bl.add_argument("--full-day-policy", choices=["single", "split"], default="single",
    ...
    bl.add_argument("--out", default="data/import",
                    help="output directory (gitignored; contains names)")
    ...
    bl.add_argument("--signal-from", default=None,
                    help="start of the classification window (default: all history)")
