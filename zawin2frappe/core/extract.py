...
AGENDA_COLUMNS = [
    "Zähler", "FK_Behandler", "FK_BehOrt", "FK_OAPraxis",
    "Datum", "VonZeit", "BisZeit",
    "FK_TagPlanTerminKategorie", "Status", "Abgesagt", "Farbe",
    "Beschreibung", "Herkunft", "ErstelltDatum", "MutDatum", "MutBehandler",
]
...
def agenda(
    ...
    *,
    ...
    def _one(table: str) -> str:
        ...
        return f"SELECT {cols}, '{table}' AS source FROM {table}{clause}"  # noqa: S608
...
def patient_activity(
    ...
    **No patient data leaves the database here** — only COUNT and the earliest
    ...
    def _one(table: str) -> str:
        ...
        """  # noqa: S608 - no user input, literals only
    ...
    if include_archive and not df.empty:
        df = (
            df.groupby(["FK_Behandler", "Datum"], as_index=False)
            .agg(am_appts=("am_appts", "sum"), pm_appts=("pm_appts", "sum"),
                 first_appt=("first_appt", "min"), last_appt=("last_appt", "max"))
        )
    return df
