...
def profile_column(table: str, column: str, dtype: str) -> dict:
	...
	df = query(f"SELECT {', '.join(agg)} FROM {t}")
...
def sample_values(table: str, column: str, n: int = 10) -> pd.DataFrame:
	...
	t, c = _quote(table), _quote(column)
	return query(f"SELECT DISTINCT TOP {int(n)} {c} FROM {t} WHERE {c} IS NOT NULL ORDER BY NEWID()")
...
def value_counts(table: str, column: str, top: int = 30) -> pd.DataFrame:
	...
	t, c = _quote(table), _quote(column)
	return query(
		f"SELECT TOP {int(top)} {c} AS value, COUNT(*) AS n FROM {t} GROUP BY {c} ORDER BY COUNT(*) DESC"
	)
