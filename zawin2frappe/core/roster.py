...
def merge_rosters(
	...
	if payroll_only.any():
		...
		for col, src in (("surname", "surname"), ("forename", "forename")):
			merged.loc[payroll_only, col] = merged.loc[payroll_only, "personnel_no"].map(extra[src])
		merged.loc[payroll_only, "service_no"] = merged.loc[payroll_only, "payroll_service_no"]
...
def classify_column(initials: str, base_initials: str | None = None, funktion: int | None = None) -> str:
...
def _base_initials(initials: list[str]) -> str:
	...
	derived = {
		i
		for i in clean
		if i.endswith(("PRO", "PR")) or i.startswith(("A-", "A_")) or (len(i) > 1 and i[-1].isdigit())
	}
...
def resolve_columns(spine: pd.DataFrame, year: int | None = None) -> pd.DataFrame:
	...
	if not overrides.empty:
		...
		log.info("applied %d curated identity links (%d columns)", len(overrides), int(hit.sum()))
	...
	cols["column_role"] = [
		...
		for i, b, f in zip(cols["initials"], cols["base_initials"], cols["funktion_code"], strict=False)
	]
