...
def resolve_ortho(annotated: pd.DataFrame) -> pd.DataFrame:
    ...
    for months in NARROWING_MONTHS[1:]:
        ...
        for row in narrowed.itertuples():
            ...
            base.loc[mask, ["ortho_share", "presence_rows", "ortho_rows"]] = [
                row.ortho_share, row.presence_rows, row.ortho_rows
            ]
    ...
    if unresolved:
        log.warning(
            ...
            len(unresolved), low * 100, high * 100, floor, sorted(unresolved),
        )
    return base
...
def refine(spine: pd.DataFrame, columns: pd.DataFrame, annotated: pd.DataFrame) -> pd.DataFrame:
    ...
    linked = columns.dropna(subset=["personnel_no"]).merge(
        ...
        left_on="behandler_id", right_on="FK_Behandler", how="left",
    )
    ...
    per_person["ortho_share"] = (
        per_person["ortho_rows"] / per_person["presence_rows"]
    ).where(per_person["presence_rows"] >= settings.get().threshold("min_presence_rows", 20))
    ...
    out.loc[unresolved & (out["ortho_share"] < settings.get().threshold("ortho", 0.50)), "discipline_resolved"] = (
        "Omnipractice"
    )
    ...
    log.info(
        "discipline: %s",
        ", ".join(
            f"{k}={v}" for k, v in out["discipline_resolved"].value_counts(dropna=False).items()
        ),
    )
    return out
