...
def build_employees(
    ...
    out = pd.DataFrame(
        {
            ...
            "last_name": surname,
            "employee_name": (
                forename.fillna("") + " " + surname.fillna("")
            ).str.strip(),
            ...
            .fillna(df["_zawin_dob"])
            .fillna(pd.Timestamp(UNKNOWN_DOB)),
            ...
            "department": (
                df["department"] if "department" in df else df["discipline_resolved"]
            ).map(department_link),
    ...
    return out.sort_values(
        "employee_number", key=lambda s: pd.to_numeric(s, errors="coerce")
