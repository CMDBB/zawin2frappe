...
def apply(spine: pd.DataFrame, annotated: pd.DataFrame, columns: pd.DataFrame) -> pd.DataFrame:
    ...
    if overruled.any():
        log.info(
            "%d clinically-placed staff look like reception on labels but keep "
            "their payroll discipline: %s",
            int(overruled.sum()), list(df.loc[overruled, "personnel_no"]),
        )
    ...
    prophy = set() if fp is None else set(
        columns.loc[
            columns["funktion_code"].eq(fp), "personnel_no"
        ].dropna()
    )
    if prophy:
    ...
    log.info(
        ...
        int(df["schedulable"].sum()), int((~df["schedulable"]).sum()),
        ", ".join(
            f"{k}={v}" for k, v in
            df.loc[~df["schedulable"], "department"].value_counts().items()
        ),
    )
    return df
