...
def fte_reconciliation(
    ...
    *,
    ...
    out = per.join(
        spine.set_index("personnel_no")[["surname", "service", "fte_pct"]]
    ).dropna(subset=["fte_pct"])
    if out.empty:
    ...
    log.info(
        "FTE check: slope %.2f half-days/FTE-point (%.1f per week at 100%%), "
        "%d of %d people within +-%.0f%%",
        slope, slope * 100 / prof.threshold("weeks_per_year", 46),
        int(out["within_tolerance"].sum()), len(out), tolerance * 100,
    )
    return out.reset_index()
...
def build(person_level: pd.DataFrame, spine: pd.DataFrame) -> pd.DataFrame:
    ...
    df["zawin_key"] = [
        keys.attested_key(z, w)
        if pd.notna(z)
        else keys.reconstructed_key(int(b), d, w)
        for z, b, d, w in zip(
    ...
    if dupes:
        raise ValueError(
            f"{dupes} duplicate custom_zawin_key values — keys are not deterministic"
        )
