...
def attested(annotated: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    rows = annotated[
        annotated["shift_kind"].isin(ASSIGNABLE_KINDS)
        & ~annotated["is_absence"]
    ]
    out = []
...
def corroboration(
    annotated: pd.DataFrame, patient: pd.DataFrame
...
def reconstruct(
    ...
    """
    absence_marked = set(
        styles.loc[styles["style"] == shifts.STYLE_ABSENCE, "FK_Behandler"]
    )
    if not absence_marked:
        return pd.DataFrame(
            columns=["behandler_id", "date", "window", "derivation", "shift_kind"]
        )
...
def build(
    ...
    combined["_rank"] = combined["derivation"].map(
        {DERIVATION_ATTESTED: 0, DERIVATION_RECONSTRUCTED: 1}
    )
    ...
    combined = combined.drop_duplicates(
        subset=["behandler_id", "date", "window"], keep="first"
...
def to_person_level(assignments: pd.DataFrame, columns: pd.DataFrame) -> pd.DataFrame:
    ...
    if orphans:
        log.info(
            ...
            "staff); dropped from the import", orphans,
        )
    ...
    out = out.drop_duplicates(
        subset=["personnel_no", "date", "window"], keep="first"
    ).drop(columns="_rank")
    log.info(
...
def collapse_daily(
    person_level: pd.DataFrame, patient: pd.DataFrame, columns: pd.DataFrame
) -> pd.DataFrame:
    ...
    share = (
        unambiguous.assign(is_am=unambiguous["window"].eq("am"))
        .groupby("personnel_no")["is_am"].mean()
    )
    ...
    prefs = []
    for idx, (pno, day, window) in enumerate(
        zip(df["personnel_no"], df["date"], df["window"], strict=False)
    ):
    ...
    df = (
        df.sort_values(
            ["personnel_no", "date", "_pref", "_att", "_am"], kind="stable"
        )
    ...
    log.info(
        ...
        before, len(df), before - len(df), df["window"].value_counts().to_dict(),
    )
    return df
