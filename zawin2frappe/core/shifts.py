...
def rules() -> list[tuple]:
	...
	if key not in _RULE_CACHE:
		_RULE_CACHE[key] = [
			(re.compile(pat, re.IGNORECASE), kind, bool(worked)) for pat, kind, worked in prof.label_rules
		]
...
def annotate(df: pd.DataFrame, label_col: str = "Beschreibung") -> pd.DataFrame:
	...
	out["shift_window"] = [shift_window(v, b) for v, b in zip(out["VonZeit"], out["BisZeit"], strict=False)]
	return out
