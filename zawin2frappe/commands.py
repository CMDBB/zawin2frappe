...
@click.option("--target", type=click.Choice(["employees", "assignments", "all"]),
              default="all", show_default=True)
@click.option("--out", default=None,
              help="Write Data Import CSVs here. Omit to load straight into Frappe.")
...
@click.option("--signal-from", default=None,
              help="Start of the window used to classify staff. Defaults to all history.")
@click.option("--signal-to", default=None)
@click.option("--full-day-policy", type=click.Choice(["single", "split"]),
              default="single", show_default=True,
              help="'single' emits at most one shift per person per day.")
@click.option("--dry-run", is_flag=True,
              help="Build and report, writing nothing.")
@pass_context
def zawin_build(context, target, out, date_from, date_to, signal_from, signal_to,
                full_day_policy, dry_run):
    ...
    try:
        ...
        result = build.run(
            sink, target=target, date_from=date_from, date_to=date_to,
            signal_from=signal_from, signal_to=signal_to,
            full_day_policy=full_day_policy, write_reports=not dry_run,
        )
