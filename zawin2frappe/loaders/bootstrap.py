...
def ensure_prerequisites(*, verbose: bool = True) -> dict:
    ...
    if not frappe.db.exists("Company", prof.company):
        ...
        try:
            frappe.get_doc({
                "doctype": "Company", "company_name": prof.company,
    ...
    if not frappe.db.exists("Department", root):
        frappe.get_doc({
            "doctype": "Department", "department_name": ROOT_DEPARTMENT,
            "company": prof.company, "is_group": 1,
        }).insert(ignore_permissions=True)
    ...
    for branch in prof.branches:
        ...
        if not frappe.db.exists("Shift Location", branch):
            frappe.get_doc({
                "doctype": "Shift Location", "location_name": branch,
