...
class FrappeDocSink:
    ...
    def write(self, doctype: str, rows: pd.DataFrame) -> None:
        ...
        payloads = [
            {c: _clean(v) for c, v in row.items()} for _, row in rows.iterrows()
        ]
        if doctype == "Employee":
    ...
    def _write_rows(self, doctype, key_field, payloads, existing, stats) -> None:
        for i, payload in enumerate(payloads):
            ...
            try:
                ...
                stats["failed"] += 1
                self.errors.append(
                    {"doctype": doctype, "key": key, "error": f"{type(exc).__name__}: {exc}"}
                )
                frappe.db.rollback()
    ...
    def _cancel(self, doctype: str, name: str) -> None:
        ...
        if "custom_zawin_key" in doc.as_dict():
            frappe.db.set_value(doctype, name, "custom_zawin_key", None,
